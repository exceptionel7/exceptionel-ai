 ...
 *
 * En production : PostgreSQL serverless via l'API REST PostgREST (compatible
 * Supabase), appelée en https natif (zéro dépendance). Configurée par :
 *   SUPABASE_URL          ex: https://xxxx.supabase.co
 *   SUPABASE_SERVICE_KEY  clé "service_role" (serveur uniquement, jamais exposée)
 *
 * En démo (aucune config) : stockage EN MÉMOIRE, pour tester sans base.
 */
...
// ---------------- Repli en mémoire (démo) ----------------
const mem = {}; // { table: [rows] }
...
async function select(table, filters) {
  if (isConfigured()) {
    return await request("GET", "/rest/v1/" + table + encodeFilters(filters), null);
  }
  return memTable(table).filter(function (r) {
    return Object.keys(filters || {}).every(function (k) { return r[k] === filters[k]; });
  });
}
